...
void luaRegister_PhysicsConstraint()
{
    ...
    egret::ScriptUtil::registerClass("PhysicsConstraint", lua_members, NULL, NULL, lua_statics, scopePath);
}
...
static PhysicsConstraint* getInstance(lua_State* state)
{
    ...
    return (PhysicsConstraint*)((egret::ScriptUtil::LuaObject*)userdata)->instance;
}
    ...
    switch (paramCount)
    {
        case 2:
        {
                ...
                (lua_type(state, 2) == LUA_TUSERDATA || lua_type(state, 2) == LUA_TTABLE || lua_type(state, 2) == LUA_TNIL))
            {
				...
				kmVec3 temp = PhysicsConstraint::centerOfMassMidpoint(param1, param2);
    ...
    switch (paramCount)
    {
        case 2:
        {
                ...
                (lua_type(state, 2) == LUA_TUSERDATA || lua_type(state, 2) == LUA_TNIL))
            {
				...
				kmQuaternion temp = PhysicsConstraint::getRotationOffset(param1, *param2);
				memcpy(returnPtr, &temp, sizeof(float) * 3);
                if (returnPtr)
                {
                    egret::ScriptUtil::LuaObject* object = (egret::ScriptUtil::LuaObject*)lua_newuserdata(state, sizeof(egret::ScriptUtil::LuaObject));
    ...
    switch (paramCount)
    {
        case 2:
        {
                ...
                (lua_type(state, 2) == LUA_TUSERDATA || lua_type(state, 2) == LUA_TNIL))
            {
				...
				kmVec3* returnPtr = new kmVec3;
				kmVec3 temp = PhysicsConstraint::getTranslationOffset(param1, *param2);
				memcpy(returnPtr, &temp, sizeof(float) * 3);
                if (returnPtr)
                {
                    egret::ScriptUtil::LuaObject* object = (egret::ScriptUtil::LuaObject*)lua_newuserdata(state, sizeof(egret::ScriptUtil::LuaObject));
