...
void luaRegister_Pass()
{
    ...
    egret::ScriptUtil::registerClass("Pass", lua_members, NULL, lua_Pass__gc, lua_statics, scopePath);
}
...
static Pass* getInstance(lua_State* state)
{
    ...
    return (Pass*)((egret::ScriptUtil::LuaObject*)userdata)->instance;
}
    ...
    switch (paramCount)
    {
        case 2:
        {
                ...
                (lua_type(state, 2) == LUA_TUSERDATA || lua_type(state, 2) == LUA_TTABLE || lua_type(state, 2) == LUA_TNIL))
            {
                // Get parameter 1 off the stack.
                bool param1Valid;
                egret::ScriptUtil::LuaArray<VertexAttributeBinding> param1 = egret::ScriptUtil::getObjectPointer<VertexAttributeBinding>(2, "VertexAttributeBinding", false, &param1Valid);
                if (!param1Valid)
                {
