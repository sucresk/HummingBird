module BlackSwan {
    ...
    export class DDSParser {
        ...
        public static parse(buffer: ArrayBuffer, loadMipmaps:boolean = true):TextureBase {
            ...
            //是否软解DXT;
            var useSoftwareSolution: boolean = true;
            if (dds.format == DDSFormat.RGB_S3TC_DXT1_FORMAT && Egret3D.ColorFormat_DXT1_RGB == 0)
                useSoftwareSolution = true;
            else if (dds.format == DDSFormat.RGBA_S3TC_DXT3_FORMAT && Egret3D.ColorFormat_DXT3_RGBA == 0)
                useSoftwareSolution = true;
            else if (dds.format == DDSFormat.RGBA_S3TC_DXT5_FORMAT && Egret3D.ColorFormat_DXT5_RGBA == 0)
                useSoftwareSolution = true;
            ...
            if (useSoftwareSolution) {
                ...
                if (FOURCC_DXT1 == fourCC)
                    texture.colorFormat = Egret3D.ColorFormat_DXT1_RGB;
                else if (FOURCC_DXT3 == fourCC)
                    texture.colorFormat = Egret3D.ColorFormat_DXT3_RGBA;
                else if (FOURCC_DXT5 == fourCC)
                    texture.colorFormat = Egret3D.ColorFormat_DXT5_RGBA;
            }
                ...
                case DDSFormat.RGBA_S3TC_DXT1_FORMAT:
                    {
                        ...
                        for (var blockIndex: number = 0; blockIndex < nCount; blockIndex++) {
                            ...
                            var globalY: number = Math.floor(blockIndex / (height / 4)) * 4;
