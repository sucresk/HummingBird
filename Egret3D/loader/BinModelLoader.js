var BlackSwan;
(function (BlackSwan) {
    var BinModelLoader = (function () {
        function BinModelLoader() {
            this._geomtryData = new BlackSwan.GeomtryData();
            this.minPos = new BlackSwan.Vector3D();
            this.maxPos = new BlackSwan.Vector3D();
        }
        Object.defineProperty(BinModelLoader.prototype, "geomtryBase", {
            get: function () {
        ...
        Object.defineProperty(BinModelLoader.prototype, "geomtryData", {
            get: function () {
        ...
        BinModelLoader.prototype.loadBinModelFromFile = function (file_url, callback_complete) {
            var _this = this;
            ...
            this._loader.onLoadComplete = function (loader) { return _this.onLoadComplete(loader); };
            this._loader.load(file_url);
        };
        BinModelLoader.prototype.onLoadComplete = function (loader) {
            this._data = loader.data;
            this.parserModelData();
            this._callback_complete(this);
        };
        BinModelLoader.prototype.parserModelData = function () {
            //this._geomtry = new SubGeomtry();
            this._data.position = 0;
            this._data.endian = BlackSwan.Endian.BIG_ENDIAN;
            var version = this._data.readInt();
            switch (version) {
                case 0x067014:
        ...
        BinModelLoader.prototype.parserModelVersion0x067014 = function (data) {
            var isCompress = data.readBoolean();
            var fileData = new BlackSwan.ByteArray();
            data.readBytes(fileData);
            if (isCompress == true) {
                fileData.uncompress("lzma");
            }
            fileData.position = 0;
            fileData.endian = BlackSwan.Endian.BIG_ENDIAN;
            var i;
            var uv;
            //读取模型名称;
            var modelName = fileData.readUTFBytes(fileData.readInt());
            //0,0偏移值;
            var vOffset = new BlackSwan.Vector3D();
            vOffset.x = fileData.readFloat();
            vOffset.z = fileData.readFloat();
            vOffset.y = fileData.readFloat();
            //缩放比;
            var vScale = new BlackSwan.Vector3D();
            ...
            //读取三角面总数;
            var triangleCount = fileData.readInt();
            //读取顶点总数;     
            var vertexCount = fileData.readInt();
            //读取纹理文件名;
            //			var textureFileName:String = fileData.readUTFBytes(fileData.readInt());
            //			//			if( textureFileName == "" )
            //			//				mesh.complete = true ;
            var sourceVertexData = new Array();
            var sourceNormalData = new Array();
            var sourceVertexColorData = new Array();
            var sourceUVData1 = new Array();
            var sourceUVData2 = new Array();
            var uTemp = 0;
            //读取顶点数组;
            for (i = 0; i < vertexCount; i++) {
                ...
                var normal = new BlackSwan.Vector3D();
                ...
                var vertexColor = new BlackSwan.Vector3D();
            ...
            for (i = 0; i < triangleCount; i++) {
                ...
                var tmpFace = new BlackSwan.FaceData();
            ...
            this._geomtryData = BlackSwan.GeomtryData.translateMaterialGroup(this._geomtryData);
            //this._geomtryData = GeomtryData.translateMaterialGroup(this._geomtryData);
            this._geomtry = new BlackSwan.SubGeomtry();
        ...
        return BinModelLoader;
    })();
    BlackSwan.BinModelLoader = BinModelLoader;
})(BlackSwan || (BlackSwan = {}));
//# sourceMappingURL=BinModelLoader.js.map