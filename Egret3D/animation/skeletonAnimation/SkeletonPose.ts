module Egret3D {
    export class SkeletonPose{
        ...
        public findJointPoseIndex(name: string): number {
        ...
        public toMatrixData(joints: Array<Joint>, target: Float32Array = null): Float32Array {
            ...
            for (var i: number = 0; i < joints.length; i++) {
                for (var j: number = 0; j < this.jointPoses.length; j++) {
                    if (this.jointPoses[j].name != joints[i].name)
                        continue;

                    this._temp_q0.fromMatrix(this.jointPoses[j].jointMatrix);
                    ...
                    target[i * 8 + 4] = this.jointPoses[j].jointMatrix.rawData[12];
                    target[i * 8 + 5] = this.jointPoses[j].jointMatrix.rawData[13];
                    target[i * 8 + 6] = this.jointPoses[j].jointMatrix.rawData[14];
                    target[i * 8 + 7] = 1;
